...
#
# Test that the number of simulations is adequate
a <-100
vector_a <- vector(length = 19)
vector_sums_WTD <- vector(length =19)
vector_sums_subdisNO3 <- vector(length =19)
vector_sums_denitr <- vector(length =19)
vector_sums_TN2 <- vector(length=19)
iteration <- 1

for (a in seq(100,1000,50)) {
  
  vector_a[iteration] <- a
  
  # Log transform (with natural log) all outputs to better visualize trends
  log1_output_WTD <- log(subset_WTD[1:a]+1)
  log1_output_denitr1 <- log(subset_denitr[1:a]+1)
  log1_output_TN2 <- log(subset_TN2[1:a]+1)
  
  # Re-run the sensitivity analysis with the log transformed outputs, and nBOOT = 500
  WTD_sens_log1_500 <- run_sensitivity(subset_inputs[1:a,], log1_output_WTD, nBOOT=0, plot = FALSE)

  vector_sums_WTD[iteration] <- sum(WTD_sens_log1_500)
  
  denitr1_sens_log1_500 <- run_sensitivity(subset_inputs[1:a,], log1_output_denitr1, nBOOT = 0, plot = FALSE)
  vector_sums_denitr[iteration] <- sum(denitr1_sens_log1_500)
  
  TN2_sens_log1_500 <- run_sensitivity(subset_inputs[1:a,], log1_output_TN2, nBOOT = 0, plot = FALSE)
  vector_sums_TN2[iteration] <- sum(TN2_sens_log1_500)
...
WTD_diffs <- vector(mode="numeric", length = (length(vector_sums_WTD)-1))
denitr_diffs <- vector(mode="numeric", length = (length(vector_sums_denitr)-1))
TN2_diffs <- vector(mode="numeric", length= (length(vector_sums_TN2)-1))

z<-1
for (z in 1:(length(vector_sums_WTD)-1)) {
  WTD_diffs[z] <- abs(vector_sums_WTD[z+1]-vector_sums_WTD[z])
  denitr_diffs[z] <- abs(vector_sums_denitr[z+1]-vector_sums_denitr[z])
  TN2_diffs[z] <- abs(vector_sums_TN2[z+1]-vector_sums_TN2[z])
...
plot(vector_a[1:18], WTD_diffs, xlab="", ylab="d(WTD Sensitivity)", xlim= c(0,1000), ylim=c(0,6.5), pch=16, xaxt="n")
axis(1, at=seq(0,1000,100), labels=FALSE)
abline(h=0, lty=3)
mtext("(a)", 3, at=(0), cex=0.6)
plot(vector_a[1:18], denitr_diffs, xlab="", ylab="d(Denitrification Sensitivity)",xlim= c(0,1000),ylim=c(0,6.5), pch=16,xaxt="n")
abline(h=0, lty=3)
axis(1, at=seq(0,1000,100), labels=FALSE)
mtext("(b)", 3, at=(0), cex=0.6)
plot(vector_a[1:18], TN2_diffs, xlab="Simulations", ylab="d(Total Nitrogen Sensitivity)",xlim= c(0,1000),ylim=c(0,6), pch=16)
axis(1, at=seq(0,1000,100), labels=FALSE)
abline(h=0, lty=3)
mtext("(c)", 3, at=(0), cex=0.6)
text(500, -2, "Simulations", str=90, xpd=NA)
dev.off()